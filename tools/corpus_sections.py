...
def _style_says_heading(p: Paragraph) -> bool:
    ...
    # Разрешаем ТОЛЬКО Heading 1–3 / Заголовок 1–3
    if re.match(r"^heading\s*[1-3]\b", low):
        return True
    if re.match(r"^заголовок\s*[1-3]\b", low):
        return True
...
def looks_like_heading(text: str, p: Optional[Paragraph] = None) -> bool:
    ...
    # 2) Нумерованный заголовок БЕЗ стиля: разрешаем ТОЛЬКО один уровень "4. Title"
    #    И запрещаем "4.17. ..." (это подпункт)
