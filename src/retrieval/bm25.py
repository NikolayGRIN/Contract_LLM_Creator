...
def smart_truncate(text: str, max_chars: int = 2600) -> str:
    ...
    cut = max_chars
    tail = t[int(max_chars * 0.65):max_chars]  
    m = None
...
def consecutive_repeats(text: str, min_len: int = 30, max_len: int = 220) -> str:
...
def filter_language(lang_req: str, lang_doc: str) -> bool:
...
def build_docs_from_rows(
    ...
    *,
    ...
    for r in rows:
        ...
        # схлопнуть повторы
        text = consecutive_repeats(text, min_len=35, max_len=220)
        ...
        if not filter_language(language_mode, lang):
            continue
...
# ============================================================
# ПОСТ-ОБРАБОТКА ПРЕЦЕДЕНТОВ: вычищаем переменные из Form
# ============================================================
...
def mask_form_variables(text: str, form: dict) -> str:
    ...
    t = consecutive_repeats(t, min_len=35, max_len=220)
    t = normalize_ws(t)
...
# ============================================================
# ДИВЕРСИФИКАЦИЯ: чтобы не брать одинаковые куски 3 раза
# ============================================================
...
def build_payment_query(language_mode: str, form: dict) -> str:
    ...
    has_prepayment = bool(prepayment_percent and int(str(prepayment_percent).replace("%", "").strip()) > 0)
...
# ============================================================
# ГОТОВАЯ ФУНКЦИЯ RETRIEVAL: DELIVERY TERMS
# ============================================================
