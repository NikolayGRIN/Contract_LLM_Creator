...
# ============================================================
# РАЗБИЕНИЕ НА ПРЕДЛОЖЕНИЯ (упрощённая эвристика)
# ============================================================
...
def _split_sentences(text: str) -> List[str]:
    ...
    Этого достаточно для юридических текстов RU/EN
    без тяжёлого NLP.
    """
    t = (text or "").strip()
    if not t:
...
def _dedupe_lines_window(text: str, window: int = 30) -> str:
    ...
    Это защищает от копипасты,
    но не "вырезает" весь повторяющийся текст подряд.
    """
...
def _remove_heading_echo(text: str) -> str:
    """
    Удаляет типичные повторы заголовков,
    которые часто попадают в тело секции
    (например, ALL CAPS строки в начале).
    """
...
# ============================================================
# АНОНИМИЗАЦИЯ (Payment Terms, версия v1+++)
# ============================================================

# Валюты кодами (ISO/часто встречающиеся)
...
# VAT/НДС со ставкой: "НДС 20%" / "VAT 20%" -> "[VAT_RATE]"
_VAT_RATE_RE = re.compile(
    r"(?:(?:ндс|vat)\s*(?:в\s*размере\s*)?)\s*(\d{1,2}(?:[.,]\d{1,2})?)\s*%",
    re.IGNORECASE,
)

# Проценты (например, 5% или 7,5%)
...
# Сроки в днях (словами) — минимально полезный набор
...
# Суммы: 1 000 000 / 1,000,000 / 1000000 / 1.000.000,00 и т.п.
_AMOUNT_RE = re.compile(
    r"(?<!\w)(?:\d{1,3}(?:[ ,.\u00A0]\d{3})+(?:[.,]\d{1,2})?|\d{4,}(?:[.,]\d{1,2})?)(?!\w)"
)

# Удаляем "расшифровки" чисел словами в скобках:
# "[AMOUNT] (пяти) банковских дней" -> "[AMOUNT] банковских дней"
...
# Если [AMOUNT] используется как "кол-во дней/дня/суток" (в т.ч. с кавычками «...» или "...")
...
# "указанный в пункте X настоящего Контракта"
...
def anonymize_payment_terms(text: str) -> Tuple[str, int]:
    ...
    # 0) Убираем расшифровки числа словами в скобках "(пяти)"
    t2, n = _PARENS_NUMBER_WORD_RE.subn(" ", t)
    reps += n
    t = t2

    # 1) VAT/НДС со ставкой: "НДС 20%" / "VAT 20%" -> "[VAT_RATE]"
    t2, n = _VAT_RATE_RE.subn("[VAT_RATE]", t)
    reps += n
    t = t2

    # 2) VAT/НДС как маркер (если ставки нет)
    ...
    # 9.1) [AMOUNT] как ссылка на пункт/раздел договора -> [CLAUSE_REF]
    t2, n = _AMOUNT_USED_AS_CLAUSE_RE.subn(r"\1 [CLAUSE_REF]", t)
    reps += n
    t = t2

    # 9.2) [AMOUNT] как срок в банковских/рабочих днях -> [TERM_DAYS]
    t2, n = _AMOUNT_USED_AS_DAYS_BANKING_RE.subn(r"[TERM_DAYS]\1", t)
    reps += n
    t = t2

    # 9.3) [AMOUNT] как срок в "днях/дня/сутках" (в т.ч. «[AMOUNT]» дней) -> [TERM_DAYS]
    t2, n = _AMOUNT_USED_AS_DAYS_GENERIC_RE.subn(r"[TERM_DAYS]\1", t)
    reps += n
    t = t2

    # 9.4) Удаляем межсекционные ссылки на пункты договора целиком
...
def truncate_sentence_safe(text: str, max_chars: int = 1500) -> str:
    ...
    не допуская обрывов на полуслове.
    """
...
class CleanReport:
    """
    Краткий отчёт о качестве очистки прецедентов.
    """
    input_count: int
    output_count: int
    dropped_empty: int
...
def clean_precedents_payment_terms(
    ...
    *,
    ...
    Вход:
        raw precedents (как пришли из BM25)

    Выход:
        - список очищенных прецедентов
        - отчёт о произведённых трансформациях
    """
